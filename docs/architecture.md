...
- **A blocked task is run again in a fresh conversation before the verdict stands** (`limits.retryBlockedInFreshChat`, 2 by default, a field on the Settings page). The runner cannot tell why a task blocked — the prompt, the machine, or the chat itself; a clean rules-engine run lost its seventh task to Copilot no longer seeing the early turns of a long conversation ("the task specification itself is absent from the conversation state"), ninety seconds after the runner had sent it. What it can do is mechanical: archive the attempt, drop the conversation pointer, open a new chat, send the contract again, run the same task. A cause in the chat goes away; a cause in the prompt or the machine blocks again in nearly the same words, and after the configured tries the task stays blocked with every attempt on its record. The register says which happened ("blocked, then done in a fresh chat" / "still blocked after N fresh chats"), and Kerrigan's brief reads the two outcomes accordingly. The same run also made every results file open with the task text as given, so the cause that was observed no longer arises.
